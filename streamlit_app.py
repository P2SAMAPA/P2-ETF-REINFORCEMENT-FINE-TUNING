...
st.set_page_config(page_title="Reinforcement Fine-Tuning (RFT)", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🎯 Reinforcement Fine-Tuning (RFT)</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">RL-based fine-tuning | Task-performance reward | Portfolio optimisation</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 RFT")
...
st.sidebar.markdown(f"**Seq len:** {config.SEQ_LEN} | **PPO clip:** {config.PPO_CLIP}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'rft_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>RFT score: {etf['rft_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized RFT Score"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized RFT Score", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🎯 Top ETFs by RFT × Momentum (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Reinforcement Fine-Tuning (RFT)** uses RL to fine‑tune a pre‑trained model.
        - The reward is **task‑performance based** (portfolio return), not preference aligned.
        - This directly optimises for downstream portfolio metrics rather than prediction error.
        - **Momentum factor:** 1 + last_return (clipped to 0.5–2.0).
        - **Final score = RFT prediction × momentum** – optimised for portfolio returns.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the RFT scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Reinforcement Fine-Tuning | RL-based optimisation for portfolio returns × momentum")
